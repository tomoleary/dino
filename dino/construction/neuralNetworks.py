...
def projected_dense(input_projector = None,last_layer_weights = None,hidden_layer_dimensions = [],\
				...
				reduced_input_dim = None,reduced_output_dim = None,activation = 'softplus'):
	"""
	...
	z =  tf.keras.layers.Dense(reduced_input_dim,activation = activation,name=name_prefix + 'dense_reduction_layer')(input_proj_layer)
	for i,hidden_layer_dimension in enumerate(hidden_layer_dimensions):
		z = tf.keras.layers.Dense(hidden_layer_dimension,activation = activation,name = name_prefix+'inner_layer_'+str(i))(z)
...
def projected_resnet(input_projector,last_layer_weights,ranks = [],compat_layer = True,\
	...
	for i,rank in enumerate(ranks):
		z = tf.keras.layers.Add(name = name_prefix+'add'+str(i))([low_rank_layer(z,rank = rank,activation = activation,\
...
def generic_dense(input_dim,output_dim,n_hidden_neurons, name_prefix = '',\
	...
	for i,n_hidden_neuron in enumerate(n_hidden_neurons):	
		z = tf.keras.layers.Dense(n_hidden_neuron, activation=activation,name = name_prefix+'inner_layer_'+str(i))(z)
