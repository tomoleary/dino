...
from rdiffLinearObservable import rdiff_linear_observable
...
parser.add_argument('-formulation',dest = 'formulation',required=False,default = 'rdiff', help="formulation name string",type=str)
...
# Instantiate convection-reaction-diffusion linear observable
...
observable = rdiff_linear_observable(mesh,**observable_kwargs)
...
prior = BiLaplacian2D(Vh[PARAMETER],gamma = args.gamma, delta = args.delta,mean = m_mean_fun.vector(),theta0 = 2.0, theta1 = 0.5,alpha = np.pi/4)
...
if args.save_as or args.save_jacobian_data:
	...
	AS_parameters['observable_constructor'] = rdiff_linear_observable
