...
from poissonLinearObservable import poisson_linear_observable
...
parser.add_argument('-formulation',dest = 'formulation',required=False,default = 'poisson', help="formulation name string",type=str)
...
# Instantiate Poisson linear observable
...
observable = poisson_linear_observable(mesh,**observable_kwargs)
# Matern Covariance Prior is instantiated here so that each process can sample m.
Vh = observable.problem.Vh
prior = BiLaplacian2D(Vh[PARAMETER],gamma = args.gamma, delta = args.delta,theta0 = 2.0, theta1 = 0.5,alpha = np.pi/4)
...
if args.save_as or args.save_jacobian_data:
	...
	AS_parameters['observable_constructor'] = poisson_linear_observable
