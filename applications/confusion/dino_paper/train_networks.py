...
parser.add_argument("-sketching_constraints", dest='sketching_constraints',required=False, default = 0,  help="execute only sketching penalzing runs?",type=int)
...
def run_just_sketch_training(settings):
	print('Running Sketching constraints!!!!')
	inner_timing_dict = {}
	initial_name = settings['network_name']
	initial_name += '_train_'+str(settings['train_data_size'])
...
if args.sketching_constraints:
	...
	all_timings.update(run_just_sketch_training(settings5050))
...
if args.sketching_constraints:
	...
	all_timings.update(run_just_sketch_training(settings10050))
...
if args.sketching_constraints and False:
	# sketching
	settingsgeneric = default_settings()
	settingsgeneric['architecture'] = 'generic_dense'
	settingsgeneric['network_name'] = 'generic_dense'

	all_timings.update(run_just_sketch_training(settingsgeneric))
...
timings_filename = 'training_timings/n_data'+str(settings5050['train_data_size'])+str(datetime.date.today())+'.pkl'
